...
public:
      ...
      .automatically_declare_parameters_from_overrides(true))
  {
    ...
    size_t valid_launch_cfg_params = 0;
    size_t ignored_launch_cfg_params = 0;
    ...
    if (valid_launch_cfg_params > 0 || ignored_launch_cfg_params > 0) {
      RCLCPP_INFO(
        get_logger(), "Loaded %zu launch CFG overrides (%zu ignored)",
        valid_launch_cfg_params, ignored_launch_cfg_params);
    }
    ...
    if (should_create_input_sub("RTCM_INPUT_ENABLED", !leader_mode_, std::nullopt, "rtcm")) {
...
private:
  struct NavCovCache
  {
    ...
    std::array<double, 9> position_enu {};
    std::array<double, 36> velocity_enu {};
  };

  struct HpPosLlhAccuracyCache
  {
    rclcpp::Time stamp;
    ubx::u4_t itow = 0;
    double h_acc_m = 0.0;
  ...
  template<typename T>
  T declare_or_get_parameter_value(const std::string & name, const T & default_value)
  {
  ...
  UBLOX_DGNSS_NODE_LOCAL
  std::string ubx_debug_topic(const std::string & base_name) const
  {
    if (leader_mode_) {
      return "/f9p/" + base_name;
    }
    return base_name;
  }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void publish_if_present(const PublisherT & publisher, const MessageT & msg) const
  {
    if (publisher) {
  ...
  template<typename PublisherT, typename MessageT>
  UBLOX_DGNSS_NODE_LOCAL
  void publish_if_present(const PublisherT & publisher, std::unique_ptr<MessageT> msg) const
  {
    if (publisher) {
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void read_leader_parameters()
  {
    leader_mode_ = declare_or_get_parameter_value<bool>("leader_mode", false);
    publish_ubx_debug_ = declare_or_get_parameter_value<bool>("publish_ubx_debug", false);
    nav_cov_timeout_sec_ =
      std::max(0.0, declare_or_get_parameter_value<double>("nav_cov_timeout_sec", 0.5));
    use_hpposllh_accuracy_fallback_ =
      declare_or_get_parameter_value<bool>("use_hpposllh_accuracy_fallback", true);
    fail_on_cfg_nak_ = declare_or_get_parameter_value<bool>("fail_on_cfg_nak", leader_mode_);

    ntrip_enabled_ = declare_or_get_parameter_value<bool>("ntrip_enabled", false);
    ntrip_host_ = declare_or_get_parameter_value<std::string>("ntrip_host", "");
    ntrip_port_ = static_cast<int>(declare_or_get_parameter_value<int64_t>("ntrip_port", 2101));
    ntrip_mountpoint_ = declare_or_get_parameter_value<std::string>("ntrip_mountpoint", "");
    ntrip_username_ = declare_or_get_parameter_value<std::string>("ntrip_username", "");
    ntrip_password_ = declare_or_get_parameter_value<std::string>("ntrip_password", "");
    ntrip_version_ = declare_or_get_parameter_value<std::string>("ntrip_version", "Ntrip/2.0");
    ntrip_use_https_ = declare_or_get_parameter_value<bool>("ntrip_use_https", false);
    ntrip_gga_interval_s_ =
      std::max(0.5, declare_or_get_parameter_value<double>("ntrip_gga_interval_s", 5.0));
    ntrip_rtcm_timeout_s_ =
      std::max(1.0, declare_or_get_parameter_value<double>("ntrip_rtcm_timeout_s", 10.0));

    RCLCPP_INFO(
      get_logger(), "leader_mode=%s publish_ubx_debug=%s ntrip_enabled=%s",
      leader_mode_ ? "true" : "false",
      publish_ubx_debug_ ? "true" : "false",
      ntrip_enabled_ ? "true" : "false");
  }
...
// once the usb is initialised this timer is disabled
  ...
  bool leader_mode_ = false;
  bool publish_ubx_debug_ = false;
  bool use_hpposllh_accuracy_fallback_ = true;
  bool fail_on_cfg_nak_ = false;
  double nav_cov_timeout_sec_ = 0.5;

  std::optional<NavCovCache> last_nav_cov_;
  std::optional<HpPosLlhAccuracyCache> last_hpposllh_accuracy_;
  std::mutex latest_gga_mutex_;
  std::optional<GgaFixCache> latest_gga_fix_;
  ...
  std::string ntrip_version_ = "Ntrip/2.0";
  bool ntrip_use_https_ = false;
  double ntrip_gga_interval_s_ = 5.0;
  double ntrip_rtcm_timeout_s_ = 10.0;
  std::atomic<bool> ntrip_stop_requested_ {false};
  std::thread ntrip_thread_;
  bool curl_global_initialized_ = false;

  mutable std::mutex usb_write_mutex_;
  std::mutex pending_cfg_mutex_;
  std::deque<PendingCfgBatch> pending_cfg_valset_batches_;
  std::string mon_ver_summary_ = "MON-VER not received yet";
  ...
  void log_usbc()
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "USB device connected: 0x%04x:0x%04x bus %03d address %03d speed %s",
      usbc_->vendor_id(),
      usbc_->product_id(),
      usbc_->bus_number(),
...
// on set parameters callback function
    ...
    std::optional<ublox_dgnss::DeviceFamily> required_family, const char * what)
  {
    ...
    RCLCPP_DEBUG(get_logger(), "input param %s: %s", param.c_str(), enabled ? "true" : "false");
    if (!enabled) {
      return false;
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  bool nav_cov_is_fresh(ubx::u4_t itow, bool require_position)
  {
    if (!last_nav_cov_) {
      return false;
    }
    if (require_position && !last_nav_cov_->pos_valid) {
      return false;
    }
    if (!require_position && !last_nav_cov_->vel_valid) {
      return false;
    }
    const double age_sec =
      (get_clock()->now() - last_nav_cov_->stamp).nanoseconds() * 1.0e-9;
    if (!std::isfinite(age_sec) || age_sec > nav_cov_timeout_sec_) {
      return false;
    }
    const auto dt_ms = std::abs(
      static_cast<int64_t>(last_nav_cov_->itow) - static_cast<int64_t>(itow));
    return dt_ms <= 200;
  }

  UBLOX_DGNSS_NODE_LOCAL
  bool hpposllh_accuracy_is_fresh(ubx::u4_t itow)
  {
    if (!last_hpposllh_accuracy_) {
      return false;
    }
    const double age_sec =
      (get_clock()->now() - last_hpposllh_accuracy_->stamp).nanoseconds() * 1.0e-9;
    if (!std::isfinite(age_sec) || age_sec > nav_cov_timeout_sec_) {
      return false;
    }
    const auto dt_ms = std::abs(
      static_cast<int64_t>(last_hpposllh_accuracy_->itow) - static_cast<int64_t>(itow));
    return dt_ms <= 200;
  }

  UBLOX_DGNSS_NODE_LOCAL
  int navsat_status_from_pvt(const ubx::nav::pvt::NavPvtPayload & payload) const
  {
    const bool valid_fix =
      payload.flags.bits.gnssFixOK &&
      !payload.flags3.bits.invalidLLH &&
      (payload.fixType == 2 || payload.fixType == 3 || payload.fixType == 4);
    if (!valid_fix) {
      return sensor_msgs::msg::NavSatStatus::STATUS_NO_FIX;
    }
    if (payload.flags.bits.carrSoln == 2) {
  ...
  UBLOX_DGNSS_NODE_LOCAL
  int gga_fix_quality_from_pvt(const ubx::nav::pvt::NavPvtPayload & payload) const
  {
    if (!payload.flags.bits.gnssFixOK || payload.flags3.bits.invalidLLH) {
      return 0;
    }
    if (payload.flags.bits.carrSoln == 2) {
      return 4;
    }
    if (payload.flags.bits.carrSoln == 1) {
      return 5;
    }
    if (payload.flags.bits.diffSoln) {
      return 2;
    }
    return 1;
  }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void fill_fix_covariance(sensor_msgs::msg::NavSatFix & msg, ubx::u4_t itow)
  {
    msg.position_covariance.fill(0.0);
    if (nav_cov_is_fresh(itow, true)) {
      msg.position_covariance = last_nav_cov_->position_enu;
      msg.position_covariance_type = sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_KNOWN;
      return;
    }
    if (!use_hpposllh_accuracy_fallback_ || !hpposllh_accuracy_is_fresh(itow)) {
      msg.position_covariance_type = sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_UNKNOWN;
      return;
    }

    const double h_acc_m = last_hpposllh_accuracy_->h_acc_m;
    const double v_acc_m = last_hpposllh_accuracy_->v_acc_m;
    if (!std::isfinite(h_acc_m) || !std::isfinite(v_acc_m) ||
      h_acc_m <= 0.0 || v_acc_m <= 0.0)
    {
      msg.position_covariance_type = sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_UNKNOWN;
      return;
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void fill_velocity_covariance(
    geometry_msgs::msg::TwistWithCovarianceStamped & msg,
    const ubx::nav::pvt::NavPvtPayload & payload)
  {
    msg.twist.covariance.fill(0.0);
    if (nav_cov_is_fresh(payload.iTOW, false)) {
      msg.twist.covariance = last_nav_cov_->velocity_enu;
      return;
    }
    const double s_acc_mps = static_cast<double>(payload.sAcc) * 1.0e-3;
    if (std::isfinite(s_acc_mps) && s_acc_mps > 0.0) {
      const double var = s_acc_mps * s_acc_mps;
      msg.twist.covariance[0] = var;
      msg.twist.covariance[7] = var;
      msg.twist.covariance[14] = var;
    }
  }

  UBLOX_DGNSS_NODE_LOCAL
  void publish_leader_outputs_from_pvt(
    ubx_queue_frame_t * f,
    const std::shared_ptr<ubx::nav::pvt::NavPvtPayload> & payload)
  {
    if (!leader_mode_ || !fix_pub_ || !fix_velocity_pub_) {
      return;
    }

    sensor_msgs::msg::NavSatFix fix_msg;
    fix_msg.header.frame_id = frame_id_;
    fix_msg.header.stamp = f->ts;
    ...
    fix_msg.status.status = navsat_status_from_pvt(*payload);
    fix_msg.latitude = static_cast<double>(payload->lat) * 1.0e-7;
    fix_msg.longitude = static_cast<double>(payload->lon) * 1.0e-7;
    fix_msg.altitude = static_cast<double>(payload->height) * 1.0e-3;
    fill_fix_covariance(fix_msg, payload->iTOW);
    fix_pub_->publish(fix_msg);

    geometry_msgs::msg::TwistWithCovarianceStamped velocity_msg;
    velocity_msg.header = fix_msg.header;
    velocity_msg.twist.twist.linear.x = static_cast<double>(payload->velE) * 1.0e-3;
    velocity_msg.twist.twist.linear.y = static_cast<double>(payload->velN) * 1.0e-3;
    velocity_msg.twist.twist.linear.z = -static_cast<double>(payload->velD) * 1.0e-3;
    fill_velocity_covariance(velocity_msg, *payload);
    fix_velocity_pub_->publish(velocity_msg);

    const int gga_fix_quality = gga_fix_quality_from_pvt(*payload);
    if (gga_fix_quality > 0 &&
      std::isfinite(fix_msg.latitude) &&
      std::isfinite(fix_msg.longitude) &&
      std::isfinite(fix_msg.altitude))
    {
      GgaFixCache gga;
      gga.latitude_deg = fix_msg.latitude;
      gga.longitude_deg = fix_msg.longitude;
      gga.hmsl_m = static_cast<double>(payload->hMSL) * 1.0e-3;
      gga.geoid_sep_m = fix_msg.altitude - gga.hmsl_m;
      gga.fix_quality = gga_fix_quality;
      gga.num_sv = payload->numSV;
      gga.hdop = std::max(0.1, static_cast<double>(payload->pDOP) * 0.01);
      std::lock_guard<std::mutex> lock(latest_gga_mutex_);
      latest_gga_fix_ = gga;
    }
  }

  UBLOX_DGNSS_NODE_LOCAL
  void update_nav_cov_cache(
    ubx_queue_frame_t * f,
    const std::shared_ptr<ubx::nav::cov::NavCovPayload> & payload)
  {
    NavCovCache cache;
    cache.stamp = f->ts;
    cache.itow = payload->iTOW;
    cache.pos_valid = static_cast<bool>(payload->posCorValid);
    cache.vel_valid = static_cast<bool>(payload->velCorValid);
    cache.position_enu.fill(0.0);
    cache.position_enu[0] = payload->posCovEE;
    cache.position_enu[1] = payload->posCovNE;
    cache.position_enu[2] = -payload->posCovED;
    cache.position_enu[3] = payload->posCovNE;
    cache.position_enu[4] = payload->posCovNN;
    cache.position_enu[5] = -payload->posCovND;
    cache.position_enu[6] = -payload->posCovED;
    cache.position_enu[7] = -payload->posCovND;
    cache.position_enu[8] = payload->posCovDD;

    cache.velocity_enu.fill(0.0);
    cache.velocity_enu[0] = payload->velCovEE;
    cache.velocity_enu[1] = payload->velCovNE;
    cache.velocity_enu[2] = -payload->velCovED;
    cache.velocity_enu[6] = payload->velCovNE;
    cache.velocity_enu[7] = payload->velCovNN;
    cache.velocity_enu[8] = -payload->velCovND;
    cache.velocity_enu[12] = -payload->velCovED;
    cache.velocity_enu[13] = -payload->velCovND;
    cache.velocity_enu[14] = payload->velCovDD;
    last_nav_cov_ = cache;
  }

  UBLOX_DGNSS_NODE_LOCAL
  void update_hpposllh_accuracy_cache(
    ubx_queue_frame_t * f,
    const std::shared_ptr<ubx::nav::hpposllh::NavHPPosLLHPayload> & payload)
  {
    if (payload->flags.bits.invalid_lat || payload->flags.bits.invalid_lon ||
      payload->flags.bits.invalid_height)
    {
      return;
    }
    const double h_acc_m = static_cast<double>(payload->hAcc) * 1.0e-4;
    const double v_acc_m = static_cast<double>(payload->vAcc) * 1.0e-4;
    if (!std::isfinite(h_acc_m) || !std::isfinite(v_acc_m) ||
      h_acc_m <= 0.0 || v_acc_m <= 0.0)
    {
      return;
    }
    last_hpposllh_accuracy_ = HpPosLlhAccuracyCache{f->ts, payload->iTOW, h_acc_m, v_acc_m};
  }

  UBLOX_DGNSS_NODE_LOCAL
  bool write_rtcm_bytes_to_device(const uint8_t * data, size_t size, const char * source) const
  {
    if (size == 0) {
      return true;
    }
    if (usbc_ == nullptr || !usbc_->dev_valid() || !usbc_->attached()) {
      if (!usb_rtcm_detached_logged_) {
        RCLCPP_WARN(get_logger(), "USB device not attached - not sending %s RTCM to device", source);
    ...
    try {
      std::lock_guard<std::mutex> lock(usb_write_mutex_);
      usbc_->write_buffer(const_cast<uint8_t *>(data), size);
      return true;
    } catch (const usb::UsbException & e) {
      RCLCPP_WARN(get_logger(), "%s RTCM write failed: %s", source, e.what());
    }
    return false;
  }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  static std::string nmea_coord(double deg, bool latitude)
  {
    const double abs_deg = std::fabs(deg);
    const int whole_deg = static_cast<int>(abs_deg);
    const double minutes = (abs_deg - whole_deg) * 60.0;
    std::ostringstream oss;
    oss << std::setfill('0') << std::setw(latitude ? 2 : 3) << whole_deg
        << std::fixed << std::setprecision(8) << std::setw(11) << minutes;
    return oss.str();
  }

  UBLOX_DGNSS_NODE_LOCAL
  static std::string add_nmea_checksum(const std::string & body)
  {
    uint8_t checksum = 0;
    ...
    std::ostringstream oss;
    oss << "$" << body << "*" << std::uppercase << std::hex << std::setfill('0')
        << std::setw(2) << static_cast<int>(checksum) << "\r\n";
    return oss.str();
  }

  UBLOX_DGNSS_NODE_LOCAL
  std::optional<std::string> build_latest_gga_sentence()
  {
    std::optional<GgaFixCache> fix;
    {
      std::lock_guard<std::mutex> lock(latest_gga_mutex_);
      fix = latest_gga_fix_;
    }
    ...
    std::tm utc {};
    gmtime_r(&now_time_t, &utc);

    const char lat_hemi = fix->latitude_deg >= 0.0 ? 'N' : 'S';
    const char lon_hemi = fix->longitude_deg >= 0.0 ? 'E' : 'W';
    std::ostringstream body;
    body << "GPGGA,"
         << std::setfill('0') << std::setw(2) << utc.tm_hour
         << std::setw(2) << utc.tm_min
         << std::setw(2) << utc.tm_sec << ".00,"
         << nmea_coord(fix->latitude_deg, true) << "," << lat_hemi << ","
         << nmea_coord(fix->longitude_deg, false) << "," << lon_hemi << ","
         << fix->fix_quality << ","
         << std::setw(2) << std::setfill('0') << std::clamp(fix->num_sv, 0, 99) << ","
         << std::fixed << std::setprecision(1) << fix->hdop << ","
         << std::fixed << std::setprecision(3) << fix->hmsl_m << ",M,"
         << std::fixed << std::setprecision(3) << fix->geoid_sep_m << ",M,,";
    return add_nmea_checksum(body.str());
  }

  UBLOX_DGNSS_NODE_LOCAL
  std::string ntrip_url() const
  {
    const std::string scheme = ntrip_use_https_ ? "https" : "http";
    std::string mountpoint = ntrip_mountpoint_;
    if (!mountpoint.empty() && mountpoint.front() == '/') {
      mountpoint.erase(mountpoint.begin());
    }
    return scheme + "://" + ntrip_host_ + ":" + std::to_string(ntrip_port_) + "/" + mountpoint;
  }

  UBLOX_DGNSS_NODE_LOCAL
  std::string ntrip_request() const
  {
    std::string mountpoint = ntrip_mountpoint_;
    if (mountpoint.empty() || mountpoint.front() != '/') {
    ...
    req << "GET " << mountpoint << " HTTP/1.1\r\n"
        ...
        << "User-Agent: NTRIP ublox_dgnss\r\n"
        << "Accept: */*\r\n";
    if (!ntrip_version_.empty()) {
      req << "Ntrip-Version: " << ntrip_version_ << "\r\n";
    }
    if (!ntrip_username_.empty() || !ntrip_password_.empty()) {
      req << "Authorization: Basic "
          << base64_encode(ntrip_username_ + ":" + ntrip_password_) << "\r\n";
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void curl_send_all(CURL * curl, const std::string & data)
  {
    size_t offset = 0;
    while (!ntrip_stop_requested_.load() && offset < data.size()) {
      size_t sent = 0;
      const CURLcode rc = curl_easy_send(
        curl, data.data() + offset, data.size() - offset, &sent);
      if (rc == CURLE_AGAIN) {
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void process_ntrip_bytes(
    const char * data, size_t size, bool & header_done, std::string & header_buffer)
  {
    if (size == 0) {
      return;
    }
    if (!header_done) {
      header_buffer.append(data, size);
      const auto end = header_buffer.find("\r\n\r\n");
      if (end == std::string::npos) {
        return;
      }
      const std::string header = header_buffer.substr(0, end);
      if (header.find("200") == std::string::npos && header.find("ICY 200 OK") == std::string::npos) {
        throw std::runtime_error("NTRIP caster rejected request: " + header);
      }
      header_done = true;
      const size_t body_offset = end + 4;
      if (body_offset < header_buffer.size()) {
        const auto body = header_buffer.substr(body_offset);
        write_rtcm_bytes_to_device(
          reinterpret_cast<const uint8_t *>(body.data()), body.size(), "NTRIP");
      }
      header_buffer.clear();
      return;
    }
    write_rtcm_bytes_to_device(reinterpret_cast<const uint8_t *>(data), size, "NTRIP");
  }

  UBLOX_DGNSS_NODE_LOCAL
  void ntrip_stream_once()
  {
    ...
    const std::string url = ntrip_url();
    curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
    curl_easy_setopt(curl, CURLOPT_CONNECT_ONLY, 1L);
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1L);
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT_MS, 5000L);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "NTRIP ublox_dgnss");
    curl_easy_setopt(curl, CURLOPT_HTTP09_ALLOWED, 1L);

    const CURLcode connect_rc = curl_easy_perform(curl);
    if (connect_rc != CURLE_OK) {
      throw std::runtime_error(curl_easy_strerror(connect_rc));
    }

    RCLCPP_INFO(get_logger(), "NTRIP connected: %s", url.c_str());
    curl_send_all(curl, ntrip_request());

    bool header_done = false;
    std::string header_buffer;
    auto last_gga = std::chrono::steady_clock::now() - std::chrono::seconds(60);
    auto last_rtcm = std::chrono::steady_clock::now();

    std::array<char, 4096> buffer {};
    while (!ntrip_stop_requested_.load()) {
      const auto now = std::chrono::steady_clock::now();
      if ((now - last_gga) >= std::chrono::duration<double>(ntrip_gga_interval_s_)) {
        if (const auto gga = build_latest_gga_sentence()) {
          curl_send_all(curl, *gga);
          RCLCPP_DEBUG(get_logger(), "NTRIP sent GGA: %s", gga->c_str());
        } else {
          RCLCPP_DEBUG(get_logger(), "NTRIP has no finite fix yet; GGA not sent");
        }
        last_gga = now;
      }

      size_t nread = 0;
      const CURLcode recv_rc = curl_easy_recv(curl, buffer.data(), buffer.size(), &nread);
      if (recv_rc == CURLE_AGAIN) {
        std::this_thread::sleep_for(20ms);
      } else if (recv_rc != CURLE_OK) {
        throw std::runtime_error(curl_easy_strerror(recv_rc));
      } else if (nread == 0) {
        throw std::runtime_error("NTRIP stream closed");
      } else {
        process_ntrip_bytes(buffer.data(), nread, header_done, header_buffer);
        if (header_done) {
          last_rtcm = now;
        }
      }

      const double rtcm_age_s =
        std::chrono::duration<double>(std::chrono::steady_clock::now() - last_rtcm).count();
      if (header_done && rtcm_age_s > ntrip_rtcm_timeout_s_) {
        throw std::runtime_error("NTRIP RTCM timeout");
      }
    }
  }

  UBLOX_DGNSS_NODE_LOCAL
  void ntrip_thread_main()
  {
    while (!ntrip_stop_requested_.load()) {
      if (ntrip_host_.empty() || ntrip_mountpoint_.empty()) {
        RCLCPP_WARN(get_logger(), "NTRIP enabled but ntrip_host or ntrip_mountpoint is empty");
      ...
      try {
        ...
        if (!ntrip_stop_requested_.load()) {
          RCLCPP_WARN(get_logger(), "NTRIP stream error: %s", e.what());
          std::this_thread::sleep_for(1s);
        }
      }
    }
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void start_ntrip_thread_if_enabled()
  {
    if (!ntrip_enabled_) {
      return;
    }
    ...
    ntrip_stop_requested_.store(false);
    ntrip_thread_ = std::thread(&UbloxDGNSSNode::ntrip_thread_main, this);
  }

  UBLOX_DGNSS_NODE_LOCAL
  void stop_ntrip_thread()
  {
    ntrip_stop_requested_.store(true);
  ...
  void rtcm_callback(const rtcm_msgs::msg::Message & msg) const
  {
    ...
    write_rtcm_bytes_to_device(msg.message.data(), msg.message.size(), "ROS topic");
  }
...
// handle out to ublox gps device to host callback
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void remember_cfg_valset_batch(const std::string & keys)
  {
    std::lock_guard<std::mutex> lock(pending_cfg_mutex_);
    pending_cfg_valset_batches_.push_back(PendingCfgBatch{get_clock()->now(), keys});
  }

  UBLOX_DGNSS_NODE_LOCAL
  std::string pop_cfg_valset_batch()
  {
    std::lock_guard<std::mutex> lock(pending_cfg_mutex_);
    if (pending_cfg_valset_batches_.empty()) {
      return "<no tracked CFG-VALSET batch>";
    }
    const auto keys = pending_cfg_valset_batches_.front().keys;
    pending_cfg_valset_batches_.pop_front();
    return keys;
  }

  UBLOX_DGNSS_NODE_LOCAL
  size_t pending_cfg_valset_count()
  {
    std::lock_guard<std::mutex> lock(pending_cfg_mutex_);
    return pending_cfg_valset_batches_.size();
  }

  UBLOX_DGNSS_NODE_LOCAL
  void cfg_val_set_poll_async_tracked(const std::string & keys)
  {
    remember_cfg_valset_batch(keys.empty() ? "<empty CFG-VALSET batch>" : keys);
    ubx_cfg_->cfg_val_set_poll_async();
  }

  UBLOX_DGNSS_NODE_LOCAL
  void handle_cfg_valset_ack(bool accepted)
  {
    const std::string keys = pop_cfg_valset_batch();
    if (accepted) {
      if (pending_cfg_valset_count() == 0) {
        RCLCPP_INFO(get_logger(), "All pending CFG-VALSET batches acknowledged");
      } else {
        RCLCPP_DEBUG(get_logger(), "CFG-VALSET ACK: %s", keys.c_str());
      }
      return;
    }

    RCLCPP_ERROR(get_logger(), "CFG-VALSET NACK for keys: %s", keys.c_str());
    RCLCPP_ERROR(get_logger(), "MON-VER: %s", mon_ver_summary_.c_str());
    RCLCPP_ERROR(
      get_logger(),
      "CFG NACK is treated as fatal when fail_on_cfg_nak is true");
    if (fail_on_cfg_nak_) {
      keep_running_ = false;
      stop_ntrip_thread();
      rclcpp::shutdown();
    ...
    try {
      ...
      for (const std::string & param_name : param_names) {
        ...
        if (++i % n == 0) {
          ...
          cfg_val_set_poll_async_tracked(item_list);
          item_list = "";
          ubx_cfg_->cfg_val_set_cfgdata_clear();
        }
...
private:
  ...
  void rtcm_queue_frame_in(rtcm_queue_frame_t * f)
  {
    ...
    // Publish the message
    publish_if_present(rtcm_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::mon::ver::MonVerPayload> payload)
  {
    ...
    mon_ver_summary_ =
      "sw_version=" + sw_version + " hw_version=" + hw_version +
      " extensions=" + version_extensions;
  }
    ...
    std::shared_ptr<ubx::mon::comms::MonCommsPayload> payload)
  {
    ...
    publish_if_present(ubx_mon_comms_pub_, std::move(msg));
  }
    ...
    std::shared_ptr<ubx::nav::velecef::NavVelECEFPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_vel_ecef_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::velned::NavVelNEDPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_vel_ned_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::timeutc::NavTimeUTCPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_time_utc_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::relposned::NavRelPosNedPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_rel_pos_ned_pub_, *msg);
  }
  ...
  void ubx_nav_pvt_pub(ubx_queue_frame_t * f, std::shared_ptr<ubx::nav::pvt::NavPvtPayload> payload)
  {
    ...
    publish_leader_outputs_from_pvt(f, payload);
    publish_if_present(ubx_nav_pvt_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::posllh::NavPosLLHPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_pos_llh_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::posecef::NavPosECEFPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_pos_ecef_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::odo::NavOdoPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_odo_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::orb::NavOrbPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_orb_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::sat::NavSatPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_sat_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::sig::NavSigPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_sig_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::hpposllh::NavHPPosLLHPayload> payload)
  {
    ...
    update_hpposllh_accuracy_cache(f, payload);
    publish_if_present(ubx_nav_hp_pos_llh_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::hpposecef::NavHPPosECEFPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_hp_pos_ecef_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::status::NavStatusPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_status_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::svin::NavSvinPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_svin_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::eoe::NavEOEPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_eoe_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::dop::NavDOPPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_dop_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::cov::NavCovPayload> payload)
  {
    ...
    update_nav_cov_cache(f, payload);
    publish_if_present(ubx_nav_cov_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::nav::clock::NavClockPayload> payload)
  {
    ...
    publish_if_present(ubx_nav_clock_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::cor::RxmCorPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_rxm_cor_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::rtcm::RxmRTCMPayload> payload)
  {
    ...
    publish_if_present(ubx_rxm_rtcm_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::measx::RxmMeasxPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_rxm_measx_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::rawx::RxmRawxPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_rxm_rawx_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::sfrbx::RxmSfrbxPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_rxm_sfrbx_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::rxm::spartn::RxmSpartnPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_rxm_spartn_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::esf::status::ESFStatusPayload> payload)
  {
    ...
    publish_if_present(ubx_esf_status_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::esf::meas::ESFMeasPayload> payload)
  {
    ...
    publish_if_present(ubx_esf_meas_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::sec::sig::SecSigPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_sec_sig_pub_, *msg);
  }
    ...
    std::shared_ptr<ubx::sec::siglog::SecSigLogPayload> payload)
  {
    ...
    // Publish the message
    publish_if_present(ubx_sec_sig_log_pub_, *msg);
  }
  ...
  void ublox_send_user_params_async()
  {
    ...
    struct StartupCfgBatch
    {
      std::string label;
      std::vector<std::string> names;
    };

    auto starts_with = [](const std::string & text, const char * prefix) {
        return text.rfind(prefix, 0) == 0;
      };

    auto startup_batch_label = [&](const std::string & name) {
        if (starts_with(name, "CFG_SIGNAL_GPS")) {
          return std::string("signal/GPS");
        }
        if (starts_with(name, "CFG_SIGNAL_GAL")) {
          return std::string("signal/GAL");
        }
        if (starts_with(name, "CFG_SIGNAL_BDS")) {
          return std::string("signal/BDS");
        }
        if (starts_with(name, "CFG_SIGNAL_GLO")) {
          return std::string("signal/GLO");
        }
        if (starts_with(name, "CFG_SIGNAL_QZSS")) {
          return std::string("signal/QZSS");
        }
        if (starts_with(name, "CFG_SIGNAL_SBAS")) {
          return std::string("signal/SBAS");
        }
        if (starts_with(name, "CFG_MSGOUT_")) {
          return std::string("msgout/") + name;
        }
        return std::string("cfg/") + name;
      };

    std::vector<StartupCfgBatch> batches;
    std::map<std::string, size_t> batch_indexes;
    size_t user_params_total = 0;
    ...
    parameter_manager_->iterate_config_items(
      [&](const ubx::cfg::ubx_cfg_item_t & ubx_ci) {
        ...
        if (p_state.param_status == PARAM_USER) {
          const std::string name = ubx_ci.ubx_config_item;
          const std::string label = startup_batch_label(name);
          auto batch_it = batch_indexes.find(label);
          if (batch_it == batch_indexes.end()) {
            batch_indexes[label] = batches.size();
            batches.push_back(StartupCfgBatch{label, {}});
            batch_it = batch_indexes.find(label);
          }
          batches[batch_it->second].names.push_back(name);
          user_params_total++;
        }
      });

    size_t user_params_sent = 0;
    size_t cfg_batches_sent = 0;
    for (const auto & batch : batches) {
      ...
      for (const auto & name : batch.names) {
        const auto * ubx_ci = parameter_manager_->find_config_item(name);
        if (ubx_ci == nullptr) {
          RCLCPP_ERROR(get_logger(), "No config item for user parameter: %s", name.c_str());
          continue;
        }

        auto p_state_maybe = parameter_manager_->get_parameter_state(name);
        if (!p_state_maybe) {
          RCLCPP_ERROR(get_logger(), "No parameter state for: %s", name.c_str());
          continue;
        }

        auto result = cfg_val_set_from_ubx_ci_p_state(*ubx_ci, p_state_maybe.value());
        if (!result.successful) {
          RCLCPP_ERROR(
            get_logger(), "user cfg val set exception for %s: %s",
      ...
      if (ubx_cfg_->cfg_val_set_cfgdata_size() > 0) {
        const std::string tracked_keys = batch.label + ": " + item_list;
        RCLCPP_DEBUG(get_logger(), "cfg_val_set_poll_async [%s]", tracked_keys.c_str());
        cfg_val_set_poll_async_tracked(tracked_keys);
        cfg_batches_sent++;
      }
      ubx_cfg_->cfg_val_set_cfgdata_clear();
    }
    ...
    RCLCPP_INFO(
      get_logger(), "Sent %zu/%zu launch CFG parameters in %zu CFG-VALSET batches",
      user_params_sent, user_params_total, cfg_batches_sent);
  }
  ...
  void ublox_send_user_cfg_to_device()
  {
    ...
    size_t i = 0;
    size_t user_params_sent = 0;
    size_t n = 1000;     // keep startup signal/profile CFG in one coherent VALSET
    parameter_manager_->iterate_config_items(
      [&](const ubx::cfg::ubx_cfg_item_t & ubx_ci) {
        auto p_state_maybe = parameter_manager_->get_parameter_state(ubx_ci.ubx_config_item);
        if (!p_state_maybe) {
